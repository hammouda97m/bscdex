...
web3 = Web3(
    Web3.HTTPProvider("https://virulent-polished-sheet.bsc.quiknode.pro/e931666f5e5490307880152c3ce4c7d3baeb17a8/"))
...
def check_and_claim_bet(bet_record):
    """Check and claim reward for a single bet (now with NET PROFIT calculation)"""
    try:
        ...
        if claimable_amount > 0:
            ...
            reward_bnb_est = web3.from_wei(claimable_amount, 'ether')
            print(f"🎉 WIN! Claimable: {reward_bnb_est:.6f} BNB for wallet {wallet_num}")
            ...
            # Calculate actual total payout
            reward_bnb_real = web3.from_wei(balance_after - balance_before, 'ether')

            # ✅ Load original bet amount using the correct column name
            df = pd.read_csv(BET_HISTORY_FILE)
            mask = (df['epoch'] == bet_record['epoch']) & (df['wallet_number'] == bet_record['wallet_number'])

            if mask.any() and 'bet_amount_bnb' in df.columns:
                original_bet_amount = df.loc[mask, 'bet_amount_bnb'].values[0]
            else:
                original_bet_amount = 0
                print(f"⚠️ Could not find original bet amount in CSV — defaulting to 0")

            # ✅ Calculate NET profit
            net_profit = float(reward_bnb_real) - float(original_bet_amount)

            print(f"✅ Real claimed payout: {reward_bnb_real:.6f} BNB | Original bet: {original_bet_amount:.6f} BNB | Net profit: {net_profit:.6f} BNB")

            send_telegram_message(f"💰🥳💰 WIN! Wallet{wallet_num} NET PROFIT: {net_profit:.6f} BNB (Epoch {epoch})💰🥳💰")
            ...
            return {
                "result": "win",
                "reward_amount": float(net_profit),
                "claimed": "yes"
            }
            ...
            # Get the original bet amount for loss calculation
            df = pd.read_csv(BET_HISTORY_FILE)
            mask = (df['epoch'] == bet_record['epoch']) & (df['wallet_number'] == bet_record['wallet_number'])

            if mask.any() and 'bet_amount_bnb' in df.columns:
                original_bet_amount = df.loc[mask, 'bet_amount_bnb'].values[0]
                loss_amount = -float(original_bet_amount)  # Make it negative
            else:
                loss_amount = 0
                print(f"⚠️ Could not find original bet amount for loss calculation")

            save_shared_loss_state(5, epoch, f"LOSS_W{wallet_num}")

            send_telegram_message(f"⛔ LOSS! Wallet{wallet_num} lost in epoch number {epoch}. ALL BOTS skip 5 rounds!")

            return {
                "result": "loss",
                "reward_amount": loss_amount,  # Now shows -0.001 instead of 0
                "claimed": "yes"
            }
