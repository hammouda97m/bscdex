...
with open("prediction_abi.json", "r") as f:
    ABI = json.load(f)
web3 = Web3(
    Web3.HTTPProvider("https://bsc-mainnet.nodereal.io/v1/a16acfa17ef245b7973338fef461c447"))
...
HOURLY_BULL_RATES = {
    ...
    12: 0.6666666666666666,    13: 0.5454545454545454,    14: 0.75,    15: 0.3333333333333333,    16: 0.5833333333333334,    17: 0.8181818181818182,
    18: 0.5,    19: 0.36363636363636365,    20: 0.5,    21: 0.5,    22: 0.5,    23: 0.5
}

# Updated feature importance weights from your new model
FEATURE_WEIGHTS = {
    "bet_ratio": 0.1933,
    "price_volatility": 0.1905,
...
def calculate_ml_prediction_score(bet_data):
    """
    ENHANCED: Calculate prediction score with SMART streak validation + BTC INFLUENCE + price streak and total change
    ...
    # === ADHD: NEW Price streak and total price change features ===
    if len(rounds_history) >= 6:
        price_changes = [rounds_history[i]['close_price_usdt'] - rounds_history[i-1]['close_price_usdt']
                         for i in range(len(rounds_history)-4, len(rounds_history))]
        price_up_streak = sum(1 for change in price_changes if change > 0)
        price_down_streak = sum(1 for change in price_changes if change < 0)
        total_price_change = sum(price_changes)
    else:
        price_up_streak = 0
        price_down_streak = 0
    ...
    # 5. Day of week (5.8% importance)
    if current_day in [4, 6]:  # Saturday, Sunday
    ...
    # === ADHD: Add price streak + mean reversion logic ===
    # If price went up 5+ rounds and total change > $7.5, expect down
    if price_up_streak >= 5 and total_price_change >= 7.5:
        score -= 0.18  # Down bias (bear)
    elif price_down_streak >= 5 and total_price_change <= -7.5:
        score += 0.18  # Up bias (bull)
    ...
    if len(rounds_history) >= 1:
        ...
        if last_move > 2.75:
            if rounds_history[-1]['price_change_usdt'] > 0:
                score -= 0.08  # Big up move = bear bias
            else:
                score += 0.08  # Big down move = bull bias
...
def place_bet(decision, current_epoch, confidence, bet_data, ml_score):
    ...
    if confidence.startswith("HIGH"):
        bet_multiplier = 0.05  # 6% of balance for high confidence (increased)
    elif confidence.startswith("MEDIUM"):
        bet_multiplier = 0.03  # 4% of balance for medium confidence (increased)
    else:
...
def main_loop():
    global bet_placed_last_round, skip_rounds_remaining

    # 🚨 LOAD SHARED LOSS STATE FIRST!
    shared_state = load_shared_loss_state()
    skip_rounds_remaining = shared_state["skip_rounds_remaining"]
    print(f"🔍 Startup: Loaded shared skip = {skip_rounds_remaining} rounds")

    # 🚨 ADD THIS: COUNTDOWN AT START OF EACH ROUND!
    ...
    while True:
        try:
            current_epoch = contract.functions.currentEpoch().call()
            # ADD THIS NEAR START OF MAIN_LOOP FUNCTION
            # Get blockchain time vs local time
            latest_block = web3.eth.get_block('latest')
            blockchain_time = latest_block.timestamp
            local_time = int(time.time())
            time_offset = blockchain_time - local_time
    ...
    # --- WAIT UNTIL 35 SECONDS BEFORE ROUND ENDS ---
    while True:
        now = int(time.time())
        time_left = end_ts - (now + time_offset)
        if time_left <= 35:
            break
        print(f"\r⏳ Waiting for 35s-before-end... {time_left:.1f}s left", end="")
        time.sleep(1)
    print("\n🚦 Starting main processing loop!")

    print(
        "\n⏳ SMART STREAK + BTC ML monitoring with 8-second price updates + 5-min BTC tracking every 0.001 seconds until 5.0001 seconds before round ends...\n")
    ...
    while True:
        ...
        try:
            ...
            btc_data = get_btc_data()
            btc_display = f" | BTC: ${btc_data['price']:,.0f} ({btc_data['change_5min']:+.2f}%)" if btc_data['price'] > 0 else ""
    ...
    if snapshot:
        ...
        print(f"⚖️ Bet Ratio: {bet_ratio:.3f}")
        ...
        if btc_data['price'] > 0:
            ...
            if abs(btc_influence) > 0.05:
                ...
                print(f"₿ BTC Impact: {btc_emoji} {btc_influence:+.2f} (${btc_data['price']:,.0f}, 5min: {btc_data['change_5min']:+.2f}%)")
        ...
        if current_streak >= 3:
            ...
            if should_use_streak_for_ml(current_epoch):
                ...
                print(f"🔥 Current Streak: {streak_emoji} {current_streak_type} x{current_streak} ❌ INVALID (Ignored in ML)")
        ...
        try:
            ...
            price_diff = live_price - lock_price
            price_emoji = "📈" if price_diff >= 0 else "📉"
            print(f"💰 Price Difference: {price_emoji} {price_diff:+.4f} USDT (Live: ${live_price:.4f} vs Lock: ${lock_price:.4f} Prev)")
        except Exception as e:
            print(f"⚠️ Could not calculate price difference: {e}")

        if len(rounds_history) >= 1:
            last_move = rounds_history[-1]['price_change_usdt']
            if abs(last_move) > 2.75:
...
if __name__ == "__main__":
    ...
    print("🛡️ Feature: Skip 10 rounds after loss (except anti-whale bets)")
    ...
    print("💥 NEW: Big move reversal (>$2.75)")
    ...
    while True:
        ...
        print("\n⏳ Waiting 900 seconds before restarting the loop...\n")
